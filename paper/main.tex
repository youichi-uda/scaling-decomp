...
\begin{itemize}
  ...
  \item \emph{Shared-$\alpha$ $F$-test.} Compare a shared-$\alpha$
        ...
        $F(1, 8) = 4.47$, two-sided $p = 0.067$. The shared-$\alpha$
        hypothesis is \textbf{not rejected at} $\alpha_\mathrm{stat}
        = 0.05$ on the N-axis alone; the $\alpha$-freedom premise of
        the per-class N-axis decomposition is therefore
        \emph{marginal} statistically.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{$\acont \approx 1.0$ may be Zipf-coupled.}
        \citet{michaud2023quantization} ties per-class $\alpha$ to the
        Zipf slope of the per-class frequency distribution; for a
        class whose targets concentrate near the unigram floor, that
        coupling produces large $\alpha$. Our $\acont = 0.98$ at
        $D{=}300\text{B}$ and $\alpha_{D,\mathrm{cont}} \in [0.77,
        0.96]$ on the D-axis are consistent with continuation
        tokens being well predicted by orthographic / morphological
        completion of a known word (whose marginal-token distribution
        is itself nearly unigram). A direct test --- subtracting a
        per-class unigram log-probability from $\Lcont$ and re-fitting
        --- would adjudicate whether our partition reduces to a
        renamed slice of Michaud's frequency partition. We did not
        run this test and flag it as the most important open
        question.
  ...
  \item \textbf{Adversarial-review verifications.} In response to
        review, we added two confirmatory artifacts now in the
        repository: \texttt{floor\_robust.py} (the shared-$E$ sweep
        and shared-$\alpha$ $F$-test reported in
        \S\ref{sec:naxis}) and \texttt{verify\_checkpoints.py} (the
        Hub-side LFS audit reported in Appendix~\ref{app:hub} via
        \texttt{x-linked-etag} HTTP headers). The unigram-LM
        subtraction test for the $\acont \approx 1.0$ /
        Zipf-coupling-or-genuine-decomposition question above
        remains the most important open follow-up we did not run.
\end{itemize}
...
coordinates and four model families, but its \emph{magnitude} is
floor-conditional on the N-axis (varies by a factor of $\sim$20
across feasible shared-$E$ values, \S\ref{sec:naxis}) and varies by
a factor of $\sim$2 across the $(N, D)$ measurement coordinates we
sampled within Pythia.
...
$E_\mathrm{cont}$ alongside the $\alpha$ values}, and run a
shared-$E$ sweep / shared-$\alpha$ $F$-test as a routine adjudicator
of how much of $\dalpha$ depends on per-class floor freedom (our
script \texttt{floor\_robust.py} runs both in $<$1\,s on cached
loss values).
