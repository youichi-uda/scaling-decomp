#!/bin/bash
# Dense D-axis ladder for 6.9B on RunPod H100.
# Add 10 new D-points to the existing 5 (10k/25k/50k/100k/143k) → 15 total D points.
set -uo pipefail
cd "$(dirname "$0")"

NEW_STEPS=(1000 2000 4000 7000 15000 22000 30000 42000 70000 130000)
IMAGE="runpod/pytorch:2.4.0-py3.11-cuda12.4.1-devel-ubuntu22.04"
KEY=~/.runpod/ssh/runpodctl-ssh-key
PUBKEY="$(cat ${KEY}.pub)"
LOG=runpod_6.9b_dense.log
...
OUT=$(runpodctl pod create --name "sd69d-$(date +%H%M%S)" \
...
for STEP in "${NEW_STEPS[@]}"; do
    ...
    $SSHO root@$SSH_IP "cd /workspace && HF_HUB_ENABLE_HF_TRANSFER=1 timeout 1500 python measure_v2.py \
        ...
        --docs 200 --seqlen 1024 --bs 4 --out $OUTF 2>&1 | tail -8" 2>&1 | tee -a "$LOG"
    ...
    if [ -f "$OUTF" ]; then
        ...
        echo "[main] MISSING $OUTF" | tee -a "$LOG"
    fi
    # Free disk: keep only small config/tokenizer blobs, remove model shards
    $SSHO root@$SSH_IP "find /root/.cache/huggingface/hub/models--EleutherAI--pythia-6.9b/blobs/ -size +100M -delete 2>/dev/null; df -h /root | tail -1" 2>&1 | tail -2 | tee -a "$LOG"
done
echo "[main] all 6.9b dense done $(date)" | tee -a "$LOG"
